...
function builtinDefs() {
  ...
  return [
    ...
    { name: "memory.search", description: "Search memory across BOTH this workspace and the org's shared knowledge, ranked together — so you find what agents in OTHER projects already learned. Semantic (by meaning) when an embeddings provider is configured, otherwise lexical (keyword). Each hit says which scope it came from. Always check here before doing research someone may already have done.", inputSchema: obj({ q: { type: "string" }, tag: { type: "string" }, scope: { type: "string", description: "workspace | shared — omit to search both." }, include_stale: { type: "boolean" } }) },
